...
const getCart = async (
    ...
    const cartCollection = admin.firestore()
...
export const storeStripeCustomerId = async (customerId: string, userUid: string) => {
    await admin
    .firestore()
    .collection(CUSTOMERS_COLLECTION)
    .doc(userUid)
...
export const deleteCart = async (userUid: string) => {
    const snapshot = await admin.firestore().collection(CART_COLLECTION)
    .doc(userUid)
    .collection(CART_COLLECTION)
    .get();
...
export const createOrder = (cart: Cart[],
    paymentId: string,
    userUid: string,
    amount: number
    ) => {
        ...
        cart.forEach((product) => { orderCart.doc(product.id.toString())
